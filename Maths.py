...
def clean_sample( sample, sigma, epsilon, iters=20 ):
    ...
    while True:
        ...
        new_median  = np.median( sample[inliers] )

        if np.abs( (new_median - median) / median ) < epsilon:
            break

    ##  Return the sample of inliers and outliers.

    median      = np.median( sample[inliers] )
    std         = np.std( sample[inliers] )
...
def convolve( x, y, F ):
    ...
    Y   = np.zeros( y.size + F.size )
    C   = np.zeros( y.size + F.size )
...
def rotate( x, y, t ):
    ...
    t   = np.pi * t
    xp  = np.cos(t) * x  -  np.sin(t) * y
    yp  = np.sin(t) * x  +  np.cos(t) * y
